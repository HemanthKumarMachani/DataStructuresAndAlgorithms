...
public class AlternatingNumberLC2149 {
    public static void main(String[] args) {
        ...
        System.out.println("Input 5: " + Arrays.toString(alternateNumbersNoEqualPositivesAndNegatives(input5)));

    }

    // For input with equal number of positive and negative elements.
    // TC: O(N), SC: O(N)
    ...
    // For input without equal number of positive and negative elements.
    public static int[] alternateNumbersNoEqualPositivesAndNegatives(int[] a) {
        if (a == null || a.length < 2)
            return a;
        ...
        int resIdx = 0, positiveIndex = 0, negativeIndex = 0;
        boolean isPositiveTurn = true;
        while (positiveIndex < positiveList.size() && negativeIndex < negativeList.size()) {
            if (isPositiveTurn) {
                resultArray[resIdx++] = positiveList.get(positiveIndex);
                positiveIndex++;
                isPositiveTurn = false;
            } else {
                resultArray[resIdx++] = negativeList.get(negativeIndex);
                negativeIndex++;
                isPositiveTurn = true;
            }
        }
